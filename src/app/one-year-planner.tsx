...
function getDateDetailsForDay(dayNumber: number, year: number = new Date().getFullYear()) {
  ...
  const weekdayName = date.toLocaleString('en-US', { weekday: 'short' });
  return { monthName, fullMonthName, dayOfMonth, weekdayName, year };
}
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: palette.background }} edges={['top', 'left', 'right']}>
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <View style={{ alignItems: 'center' }}>
              ...
              <Text style={{ fontSize: 17, fontWeight: '900', color: palette.textPrimary, marginTop: 1 }}>
                {dateDetails.weekdayName}, {dateDetails.monthName} {dateDetails.dayOfMonth}
              </Text>
            </View>
          ...
          <View
            style={{
              ...
              borderRadius: 20,
              borderWidth: 1,
              borderColor: isDayCompleted ? palette.accentGreen : palette.cardBorder,
              padding: 18,
              shadowColor: '#000',
          ...
          >
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              ...
              <View style={{ backgroundColor: isDayCompleted ? palette.accentGreenLight : 'rgba(217, 119, 6, 0.12)', paddingHorizontal: 9, paddingVertical: 3, borderRadius: 10 }}>
                <Text style={{ fontSize: 11, fontWeight: '800', color: isDayCompleted ? palette.accentGreen : palette.accentGold }}>
                  {isDayCompleted ? '✓ Completed' : '4 Portions'}
                </Text>
              </View>
            </View>
      ...
      {viewMode === 'calendar' && (() => {
        ...
        const isSelectedDone = completedDaysArray.includes(selectedDay);
        const selectedDayDetails = getDateDetailsForDay(selectedDay, calendarYear);
        ...
        return (
          <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 50 }}>
            ...
            {/* Weekday Row Header */}
            <View style={{ flexDirection: 'row', paddingHorizontal: 10, paddingVertical: 8, backgroundColor: isDark ? '#171412' : '#F9F6F0', borderBottomWidth: 1, borderBottomColor: palette.border }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 8, paddingVertical: 8, backgroundColor: palette.card }}>
              ...
              {Array.from({ length: firstDayIndex }).map((_, idx) => (
                <View key={`empty-${idx}`} style={{ width: '14.28%', height: 54, padding: 2 }} />
              ))}
                ...
                return (
                  <View key={`day-${dNum}`} style={{ width: '14.28%', padding: 2 }}>
                    <TouchableOpacity
                      ...
                      style={{
                        ...
                        backgroundColor: isDone
                          ? (isDark ? 'rgba(16, 185, 129, 0.18)' : '#E6F4EA')
                          : isToday
                    ...
                    >
                      ...
                      {isDone ? (
                        <View style={{ width: 18, height: 18, borderRadius: 9, backgroundColor: palette.accentGreen, alignItems: 'center', justifyContent: 'center' }}>
                          <Check size={11} color="#FFFFFF" />
                        </View>
                      ) : (
                        <Text style={{ fontSize: 9, fontWeight: '700', color: palette.textMuted }}>
                          D{planDay}
                        </Text>
                      )}
                    </TouchableOpacity>
            ...
            <View style={{ marginHorizontal: 16, marginTop: 14, backgroundColor: palette.card, borderRadius: 16, borderWidth: 1, borderColor: palette.cardBorder, padding: 16 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <View>
                  <Text style={{ fontSize: 11, fontWeight: '800', color: palette.accentGold, textTransform: 'uppercase' }}>
                    Selected Date
                  </Text>
                  <Text style={{ fontSize: 16, fontWeight: '900', color: palette.textPrimary, marginTop: 1 }}>
                    Day {selectedDay} • {selectedDayDetails.monthName} {selectedDayDetails.dayOfMonth}
                  </Text>
                </View>

                <TouchableOpacity
                  onPress={() => handleToggleComplete(selectedDay)}
                  style={{ flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: isSelectedDone ? palette.accentGreenLight : palette.inputBg, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 12 }}
                >
                  <CheckCircle2 size={15} color={isSelectedDone ? palette.accentGreen : palette.textMuted} />
                  <Text style={{ fontSize: 11, fontWeight: '800', color: isSelectedDone ? palette.accentGreen : palette.textSecondary }}>
                    {isSelectedDone ? 'Completed' : 'Mark Done'}
                  </Text>
                </TouchableOpacity>
              </View>

              {/* 4 Portions Quick View */}
              <View style={{ gap: 6 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: palette.inputBg, paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8 }}>
                  <Text style={{ fontSize: 11, fontWeight: '700', color: palette.textSecondary }}>1. OT: {selectedReading.oldTestament.displayText}</Text>
                  <TouchableOpacity onPress={() => handleOpenScriptureInBible(selectedReading.oldTestament)}>
                    <Text style={{ fontSize: 11, fontWeight: '800', color: palette.accentGold }}>Read →</Text>
                  </TouchableOpacity>
                </View>

                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: palette.inputBg, paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8 }}>
                  <Text style={{ fontSize: 11, fontWeight: '700', color: palette.textSecondary }}>2. NT: {selectedReading.newTestament.displayText}</Text>
                  <TouchableOpacity onPress={() => handleOpenScriptureInBible(selectedReading.newTestament)}>
                    <Text style={{ fontSize: 11, fontWeight: '800', color: palette.accentGreen }}>Read →</Text>
                  </TouchableOpacity>
                </View>

                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: palette.inputBg, paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8 }}>
                  <Text style={{ fontSize: 11, fontWeight: '700', color: palette.textSecondary }}>3. Ps: {selectedReading.psalm.displayText}</Text>
                  <TouchableOpacity onPress={() => handleOpenScriptureInBible(selectedReading.psalm)}>
                    <Text style={{ fontSize: 11, fontWeight: '800', color: '#3498DB' }}>Read →</Text>
                  </TouchableOpacity>
                </View>

                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: palette.inputBg, paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8 }}>
                  <Text style={{ fontSize: 11, fontWeight: '700', color: palette.textSecondary }}>4. Prov: {selectedReading.proverb.displayText}</Text>
                  <TouchableOpacity onPress={() => handleOpenScriptureInBible(selectedReading.proverb)}>
                    <Text style={{ fontSize: 11, fontWeight: '800', color: '#9B59B6' }}>Read →</Text>
                  </TouchableOpacity>
                </View>
              </View>

              {/* Switch to Today's Reading Tab */}
              <TouchableOpacity
                onPress={() => {
                  triggerLightHaptic();
                  setViewMode('today');
              ...
              >
                <Text style={{ fontSize: 12, fontWeight: '800', color: palette.accentGold }}>
                  Open Day {selectedDay} in Today's Walk →
                </Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
